package merkleblock

import (
	"bytes"
	"chapter13/utils"
	"encoding/hex"
	"testing"
)

func TestParse(t *testing.T) {
	rawMerkleBlock, _ := hex.DecodeString("")
	mb := MerkleBlock{}
	err := mb.Parse(rawMerkleBlock)
	if err != nil {
		t.Fatalf("Parse error: %v", err)
	}

	version := 0x20000000
	if mb.Version != version {
		t.Fatalf("Version mismatch: got %x, want %x", mb.Version, version)
	}

	merkleRootHex := "ef445fef2ed495c275892206ca533e7411907971013ab83e3b47bd0d692d14d4"
	merkleRoot, _ := hex.DecodeString(merkleRootHex)
	merkleRoot = utils.ReverseBytes(merkleRoot)
	if !bytes.EqualFold(mb.MerkleRoot, merkleRoot) {
		t.Fatalf("MerkleRoot mismatch: got %x, want %x", mb.MerkleRoot, merkleRoot)
	}

	prevBlockHex := "df3b053dc46f162a9b00c7f0d5124e2676d47bbe7c5d0793a500000000000000"
	prevBlock, _ := hex.DecodeString(prevBlockHex)
	prevBlock = utils.ReverseBytes(prevBlock)
	if !bytes.EqualFold(mb.PrevBlockHash, prevBlock) {
		t.Fatalf("PrevBlock mismatch: got %x, want %x", mb.PrevBlockHash, prevBlock)
	}

	timestampHex, _ := hex.DecodeString("dc7c835b")
	timestamp := utils.LittleEndianToInt(timestampHex)
	if mb.Timestamp != timestamp {
		t.Fatalf("Timestamp mismatch: got %d, want %d", mb.Timestamp, timestamp)
	}

	bitsHex, _ := hex.DecodeString("67d8001a")
	bits := utils.BytesToInt(bitsHex)
	if mb.Bits != bits {
		t.Fatalf("Bits mismatch: got %d, want %d", mb.Bits, bits)
	}

	nonceHex, _ := hex.DecodeString("c157e670")
	nonce := utils.BytesToInt(nonceHex)
	if mb.Nonce != nonce {
		t.Fatalf("Nonce mismatch: got %d, want %d", mb.Nonce, nonce)
	}

	totalHex, _ := hex.DecodeString("bf0d0000")
	total := utils.LittleEndianToInt(totalHex)
	if mb.TotalTx != total {
		t.Fatalf("Total mismatch: got %d, want %d", mb.TotalTx, total)
	}

	hashesHex := []string{
		"ba412a0d1480e370173072c9562becffe87aa661c1e4a6dbc305d38ec5dc088a",
		"7cf92e6458aca7b32edae818f9c2c98c37e06bf72ae0ce80649a38655ee1e27d",
		"34d9421d940b16732f24b94023e9d572a7f9ab8023434a4feb532d2adfc8c2c2",
		"158785d1bd04eb99df2e86c54bc13e139862897217400def5d72c280222c4cba",
		"ee7261831e1550dbb8fa82853e9fe506fc5fda3f7b919d8fe74b6282f92763ce",
		"f8e625f977af7c8619c32a369b832bc2d051ecd9c73c51e76370ceabd4f25097",
		"c256597fa898d404ed53425de608ac6bfe426f6e2bb457f1c554866eb69dcb8d",
		"6bf6f880e9a59b3cd053e6c7060eeacaacf4dac6697dac20e4bd3f38a2ea2543",
		"d1ab7953e3430790a9f81e1c67f5b58c825acf46bd02848384eebe9af917274c",
		"dfbb1a28a5d58a23a17977def0de10d644258d9c54f886d47d293a411cb62261",
	}

	hashes := make([][]byte, len(hashesHex))
	for i, h := range hashesHex {
		hash, _ := hex.DecodeString(h)
		hashes[i] = utils.ReverseBytes(hash)
	}

	for i, h := range hashes {
		if !bytes.EqualFold(mb.Hashes[i], h) {
			t.Fatalf("Hash mismatch: got %x, want %x", mb.Hashes[i], h)
		}
	}

	flags, _ := hex.DecodeString("b55635")
	if !bytes.EqualFold(mb.Flags, flags) {
		t.Fatalf("Flags mismatch: got %x, want %x", mb.Flags, flags)
	}
}

func TestIsValid(t *testing.T) {
	rawMerkleBlock, _ := hex.DecodeString("")
	mb := MerkleBlock{}
	err := mb.Parse(rawMerkleBlock)
	if err != nil {
		t.Fatalf("Parse error: %v", err)
	}

	valid, err := mb.IsValid()
	if err != nil {
		t.Fatalf("IsValid error: %v", err)
	}

	if !valid {
		t.Fatalf("MerkleBlock is not valid")
	}
}
